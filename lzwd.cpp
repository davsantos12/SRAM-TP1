...
size_t compress(const std::string& block, bool clean_dict, size_t dict_size, int dict_type, int& dict_full, int& fail_insert) {
    ...
    while (ind < block.length()) {
        ...
        int j = 0;
        while (j < i) {
            int inserted = insert(Pa + block.substr(ind, j + 1), clean_dict, dict_size, dict_type, dict_full, fail_insert);
            if (inserted > 0) {
                added_patterns++;
                added = true;
                // std::cout << "added patterns: " << added_patterns << std::endl;
                // std::cout << "Pattern: " << Pa + Pb << std::endl;
            }

            j++;
        }
        if (added) {
            check_compression.push_back(Pa + Pb);
            added = false;
        }
